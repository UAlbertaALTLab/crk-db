...
function compareAnalysis( unparsed, parsed ) {
  if (!unparsed) {
    return false
  }
  const [ prefixStr, suffixStr ] = unparsed.split(parsed[1]);
  if (suffixStr == undefined) {
    // This happens when the stem doesn't match
    return false
  }
  const prefixTagsU = new Set (prefixStr.split('+').filter(Boolean).map(entry => entry+'+'))
  const suffixTagsU = new Set (suffixStr.split('+').filter(Boolean).map(entry => '+'+entry))
  const prefixTagsP = new Set (parsed[0])
  const suffixTagsP = new Set (parsed[2])
  return prefixTagsU.isSubsetOf(prefixTagsP) &&
    prefixTagsP.isSubsetOf(prefixTagsU) &&
    suffixTagsU.isSubsetOf(suffixTagsP) &&
    suffixTagsP.isSubsetOf(suffixTagsU);
}
...
function addFstInfo(entries) {
  ...
  for (const entry of entries) {
    ...
    entry.fst       ??= {};
    entry.fst.head ??= entry.head.proto ?? entry.head.sro;
    ...
    let matches = fst.lookup_lemma_with_affixes(entry.fst.head);

    const hasAnnotatedFST = matches.filter(analysis => compareAnalysis(entry.candidateFST,analysis));

    if (hasAnnotatedFST?.length == 1) {
      [entry.fst.analysis] = hasAnnotatedFST;
...
function aggregateEntry(entry) {
  ...
  entry.category = extractCategory(entry);
  ...
  entry.rw_indices = {
    "CW": Array.from(new Set(cw?.rw_indices ?? [])),
    "MD": Array.from(new Set(Object.groupBy(md?.semanticIndices ?? [],
      (element) => {
        return (md?.semanticIndices ?? []).filter((index) => index.startsWith(element)).length
      })[1]
    ))
  };
...
export default async function aggregate(dbPath, outPath = `out.ndjson`) {
  ...
  entries = entries.filter(entry => {
    return (!entry.dataSources.CW || entry.dataSources.CW.dialects.includes(`plai1258`))
  });
