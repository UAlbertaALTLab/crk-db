...
function addFSTInfo(entries) {
  ...
  for (const entry of entries) {
    ...
    entry.fst.stem     ??= entry.dataSources.ALT?.fst.stem ?? entry.fst.stem ?? entry.stem?.proto;
    entry.fst.analysis ??= entry.analysis;

    if (entry.fst.analysis) continue;
 ...
 * @returns {Object}
 */
function parseAnalysis([prefixTags, lemma, suffixTags]) {

  const [pos, subclass] = suffixTags;
