...
export class ToolboxEntry {
  ...
  constructor(entryText) {
    ...
    this.features    = this.getLineByType(`gr1`);
    this.glosses     = this.getLinesByType(`gl`);
    this.new         = typeof this.getLineByType(`new`) !== `undefined`;
    this.pos         = this.getLinesByType(`ps`);
    // TODO: @fbanados awaiting for resolution of UAlbertaALTLab/crk-db#120
    this.pos         = this.pos.map( entry => entry.replace(/[{()}]/g,'') )
    const dINM       = this.getLinesByType(`dINM`);
    // TODO: @fbanados we need to finalize a way to handle these in morphodict without breaking behaviour
    if(dINM?.length > 0  && this.pos[0] == 'INM') {
      this.dINM = dINM;
    }
  ...
  static parseLine(line, index) {
    ...
    if (!match) {
      throw new Error([
        "Couldn't parse line in entry:",
        line
      ].join(" "))
    }
    const { text = ``, type } = match.groups;
