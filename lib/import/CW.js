...
function createKey(proto, pos) {
  proto = proto.replace(/\s+/gu, `_`);
  return `${ proto }@${ pos }`;
}
...
class DatabaseIndex extends Map {
  ...
  add(entry) {
    ...
    const key           = createKey(entry.lemma.proto, entry.pos);
...
export default async function importCW(cwPath, dbPath) {
  ...
  for (const cwEntry of cwEntries) {
    ...
    const key     = createKey(cwEntry.lemma.proto, cwEntry.pos);
