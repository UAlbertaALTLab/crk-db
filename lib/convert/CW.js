...
export class Entry {
  ...
  constructor({
    ...
    this.rw_indices = Object.groupBy(rw_indices, (element) => {
      return rw_indices.filter((index) => index.startsWith(element)).length
    })[1];
    ...
    this.head = {
      proto: sro.trim(), // the CW \sro field is actually the proto-orthography
      sro:   proto2sro(sro.trim()),
      syll,
    };
  ...
  #extractNote(definition) {
    ...
    return {
      ...
      definition:    [textBefore, textAfter].map(str => str.trim()).join(` `).trim(),
    };
