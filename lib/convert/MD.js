...
class Entry {
  ...
  constructor({
    ...
    if (CW_Lemma && CW_Lemma !='+?') {
      const lemmas = CW_Lemma.split(' ;; ');
      const analizes = Analysis?.split(' ;; ') ?? [''];
      this.mapping =  {
        analysis:   analizes[0],
    ...
    if (Grammatical_Note) {
      this.notes ??= [];
      this.notes.push({
        noteType: `grammatical_note`,
        text: Grammatical_Note
      })
    }

    this.senses = Entry.#splitDefinition(MeaningInEnglish) //CHANGED
      .map(definition => ({

        definition:  Entry.#neutralizePronoun(definition),
        original: definition,
      }));
  ...
  static #splitDefinition(definition) {
    ...
    .map(str => str.replace(/[.,]$/u, ``)); // remove trailing commas - CHANGE
  }

  static #neutralizePronoun(string) { //CHANGED
    return string
     .replace(/\bhe\b/gi,'s/he')
     .replace(/\bhim\b/gi,'him/her')
     .replace(/\bhis\b/gi,'his/her')
     .replace(/\bhimself\b/gi,'himself/herself')
  }  
