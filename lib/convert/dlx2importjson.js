...
function convertEntry({
  ...
  senses = senses.map(({ definition, original, sources }) => {
    ...
    // TODO: Build cross-references, examples, and notes back into the display definition.
...
/**
 * Extracts parenthetical text from a definition, excluding certain types of parenthetical text.
 * @param {String} definition The definition to extract parenthetical text from.
 * @returns {Object}          Returns an Object with the parenthetical text and the remainder of the definition.
 */
function extractParentheticalText(definition) {

  const matches = definition.matchAll(parentheticalTextRegExp);
