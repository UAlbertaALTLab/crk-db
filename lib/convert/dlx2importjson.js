...
function createCoreDefinition(definition) {
  ...
  while ((match = parentheticalRegExp.exec(coreDefinition)) !== null) {
    ...
    if (
      ...
      pronouns.has(parenText)        // allow pronouns
    ) {
      continue;
    }
  ...
  return cleanText(coreDefinition);

}

function createSemanticDefinition(definition, literalMeaning) {
  ...
  if (literalMeaning) semanticDefinition += `[literally: ${ literalMeaning }]`;

  return cleanText(semanticDefinition);
...
function convertEntry({
  ...
  senses = senses.map(({ definition, original, sources }) => {
    ...
    const semanticDefinition = createSemanticDefinition(definition, literalMeaning);
