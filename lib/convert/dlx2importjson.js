...
// TEMPORARY CODE FOR PARENTHETICALS
// const ws = fs.createWriteStream(`parentheses.tsv`);
// const parentheticals = new Map;
...
function convertEntry(
  ...
  senses = senses.map(({ definition, original, sources }) => {
    ...
    // TEMP
    // Find parenthetical text within cleaned definition.
    const matches = Array.from(definition.matchAll(/\((?<text>.+?)\)/gu));

    for (const match of matches) {

      const { text } = match.groups;

      if (parentheticals.has(text)) {
        const count = parentheticals.get(text);
        parentheticals.set(text, count + 1);
      } else {
        parentheticals.set(text, 1);
      }
...
export default async function dlx2importjson(inputPath, outputPath = `out.json`) {
  ...
  Array.from(parentheticals.entries())
  .sort(([, a], [, b]) => compare(b, a)) // sort based on counts
  .forEach(([text, count]) => {
    ws.write(`${ text }\t${ count }\r\n`);
  });
